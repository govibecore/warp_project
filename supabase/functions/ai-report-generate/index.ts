...
function deterministicReport(
  ...
  const strongest = sorted[0]?.[0] || 'mathematicalReasoning';
  const weakest = sorted[sorted.length - 1]?.[0] || 'systemsThinking';
  ...
  return {
    student_variant: {
      ...
      keyStrengths: [
        ...
        'Demonstrated persistence through all 30 benchmark scenarios.',
...
function buildPrompt(
  ...
  return `Generate a dual-audience STEM diagnostic report for:
...
Output strict JSON:
{
  ...
  "parent_variant": {
    "overallAssessment": "string (3 paragraphs, honest executive reality check comparing to Singapore/China)",
    "keyStrengths": ["3 strings"],
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { error: insertError } = await supabase.from('reports').upsert({
      ...
      student_variant: reportData.student_variant,
      parent_variant: reportData.parent_variant,
    ...
    try {
      await supabase.channel(`report:${assessmentId}`).send({
        type: 'broadcast', event: 'section',
        payload: { section: 'complete', report: reportData }
      });
    } catch { /* non-fatal */ }
