  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (type === 'register') {
      ...
      if (signUpErr) {
        ...
        setAwaitingOtp(true);
        setNotice(`We sent an 8-digit code to ${email.trim()}. Enter it below to confirm your account.`);
      }
    } else {
    ...
    <form onSubmit={handleSubmit} className="flex w-full flex-col gap-3">
      ...
      <div className="flex flex-col items-center gap-1.5 text-center mb-1">
        <h1 className="text-lg sm:text-xl font-display font-bold tracking-tight text-foreground">
          {awaitingOtp ? 'Verify your email' : type === 'login' ? 'Welcome back' : 'Create your account'}
        </h1>
        <p className="text-muted-foreground text-[11px] text-balance">
          {awaitingOtp
            ? 'Enter the 8-digit confirmation code sent to your inbox'
            : type === 'login'
      ...
      {awaitingOtp ? (
        <Field label="Confirmation Code" htmlFor="otpCode">
          <Input
            id="otpCode"
            type="text"
            maxLength={8}
      ...
      <Button
        ...
        block
        size="md"
        className="w-full mt-2 cursor-pointer font-semibold text-xs tracking-wider uppercase relative overflow-hidden transition-colors"
      >
        {isSuccess ? (
